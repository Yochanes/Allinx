...
public final class HighLevelEncoder {
    ...
    /* JADX INFO: compiled from: Proguard */
    public class C49461 implements Comparator<State> {
        public C49461() {
        }
    ...
    private void updateStateForChar(State state, int i, Collection<State> collection) {
        char c2 = (char) (this.text[i] & UnsignedBytes.MAX_VALUE);
        ...
        for (int i2 = 0; i2 <= 4; i2++) {
            ...
            if (i3 > 0) {
                ...
                if (!z2 || i2 == state.getMode() || i2 == 2) {
                    collection.add(stateEndBinaryShift.latchAndAppend(i2, i3));
                }
                if (!z2 && SHIFT_TABLE[state.getMode()][i2] >= 0) {
    ...
    */
    public BitArray encode() {
        ...
        while (true) {
            byte[] bArr = this.text;
            if (i2 >= bArr.length) {
                return ((State) Collections.min(collectionSingletonList, new C49461())).toBitArray(this.text);
            }
            int i3 = i2 + 1;
