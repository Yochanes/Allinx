...
public final class WhiteRectangleDetector {
    ...
    private ResultPoint[] centerEdges(ResultPoint resultPoint, ResultPoint resultPoint2, ResultPoint resultPoint3, ResultPoint resultPoint4) {
        ...
        return x < ((float) this.width) / 2.0f ? new ResultPoint[]{new ResultPoint(x4 - 1.0f, y5 + 1.0f), new ResultPoint(x2 + 1.0f, y3 + 1.0f), new ResultPoint(x3 - 1.0f, y4 - 1.0f), new ResultPoint(x + 1.0f, y2 - 1.0f)} : new ResultPoint[]{new ResultPoint(x4 + 1.0f, y5 + 1.0f), new ResultPoint(x2 + 1.0f, y3 - 1.0f), new ResultPoint(x3 - 1.0f, y4 + 1.0f), new ResultPoint(x - 1.0f, y2 - 1.0f)};
    }

    private boolean containsBlackPoint(int i, int i2, int i3, boolean z2) {
        if (z2) {
            while (i <= i2) {
