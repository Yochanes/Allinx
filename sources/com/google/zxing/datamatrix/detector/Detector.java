...
public final class Detector {
    ...
    private static ResultPoint moveAway(ResultPoint resultPoint, float f, float f2) {
        ...
        return new ResultPoint(x < f ? x - 1.0f : x + 1.0f, y2 < f2 ? y2 - 1.0f : y2 + 1.0f);
    }
    ...
    private ResultPoint[] shiftToModuleCenter(ResultPoint[] resultPointArr) {
        ...
        ResultPoint resultPointMoveAway = moveAway(resultPoint, x, y2);
        ResultPoint resultPointMoveAway2 = moveAway(resultPoint2, x, y2);
        ResultPoint resultPointMoveAway3 = moveAway(resultPoint3, x, y2);
        ResultPoint resultPointMoveAway4 = moveAway(resultPoint4, x, y2);
        int i3 = i2 * 4;
        int i4 = i * 4;
    ...
    private int transitionsBetween(ResultPoint resultPoint, ResultPoint resultPoint2) {
        ...
        int i = 0;
        boolean z2 = Math.abs(iMin - y2) > Math.abs(x2 - x);
        if (z2) {
            y2 = x;
            x = y2;
            iMin = x2;
        ...
        int iAbs2 = Math.abs(iMin - y2);
        int i2 = (-iAbs) / 2;
        int i3 = y2 < iMin ? 1 : -1;
        int i4 = x >= x2 ? -1 : 1;
        boolean z3 = this.image.get(z2 ? y2 : x, z2 ? x : y2);
        while (x != x2) {
            boolean z4 = this.image.get(z2 ? y2 : x, z2 ? x : y2);
            if (z4 != z3) {
                i++;
                z3 = z4;
            }
