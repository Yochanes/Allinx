...
public class EdgeToEdgeUtils {
    ...
    private static int getNavigationBarColor(Context context, boolean z2) {
        if (z2 && Build.VERSION.SDK_INT < 27) {
            return ColorUtils.m7560i(MaterialColors.getColor(context, R.attr.navigationBarColor, -16777216), 128);
        }
        if (z2) {
            return 0;
        }
    ...
    public static void setLightNavigationBar(@NonNull Window window, boolean z2) {
        new WindowInsetsControllerCompat(window, window.getDecorView()).m7956e(z2);
    }

    public static void setLightStatusBar(@NonNull Window window, boolean z2) {
        new WindowInsetsControllerCompat(window, window.getDecorView()).m7957f(z2);
    }
