...
## V-09 (High, conditional): GeeTest GT4 может уйти в HTTP при неверной/неполной конфигурации `initGeetest4`
- **Факт в SDK-ассете:** `Config.prototype.protocol: 'http://'` по умолчанию.
- **Факт в инициализации:** только если передан `userConfig.https` -> `config.protocol = 'https://'`; иначе при отсутствии `userConfig.protocol` берется `window.location.protocol + '//'`.
- **Почему это уязвимость:** при misconfig (или при переиспользовании `gt4.js` без явной HTTPS-настройки) запросы GeeTest уходят в чистом HTTP.
- **Реальная атака:** MITM в публичной сети подменяет ответы `/load`/статические скрипты captcha, что позволяет:
  1. ломать/обходить антибот-проверку;
  2. подсовывать вредоносный JS в captcha webview-контекст;
  3. фальсифицировать UX проверки (fail-open сценарии в бизнес-логике интегратора).
- **Критерии подтверждения:** достаточно любого code path, где `initGeetest4(userConfig)` вызывается без `https:true` и без жестко заданного `protocol:'https://'`.
- **Примечание по текущему приложению:** в части native SDK видно принудительную установку `"protocol":"https://"` при подготовке конфигурации webview; это снижает риск в *этом* code path, но уязвимость остается **реальной и эксплуатационной** для любых альтернативных/ошибочных инициализаций.
...
## A-05: GeeTest HTTP downgrade / captive-portal MITM
1. Пользователь подключается к контролируемой злоумышленником сети (open Wi‑Fi/captive portal/evil twin).
2. Ветка инициализации captcha запускается без `https:true` (или без жесткого `protocol:'https://'`).
3. Трафик GT4 (`/load`, gct/static) проходит по HTTP и может быть подменен.
4. Атакер инжектит/меняет JS-ответы, инициируя fail-open/подмену поведения captcha.
**Импакт:** обход anti-bot контроля, автоматизация abuse-операций, риск дальнейшей webview-эскалации через подмененный JS.
...
5. Для GeeTest закрепить transport policy: всегда передавать `https:true` + принудительный `protocol:'https://'` + runtime assertion/telemetry на отклонение.
